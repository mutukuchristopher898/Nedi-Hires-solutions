    ...
    <div>
      ...
      {account.suspendedAt && !account.anonymisedAt && (
        <p className="mt-4 rounded-md bg-amber/10 px-4 py-3 text-sm text-amber">
          ...
          {account.suspensionReason ? ` — ${account.suspensionReason}` : ""}. They cannot make new
          bookings, send enquiries or list vehicles. An existing signed-in session stays valid
          until it expires, but every write is refused.
        </p>
      )}
